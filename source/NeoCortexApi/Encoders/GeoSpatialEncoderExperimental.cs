...
namespace NeoCortexApi.Encoders
{
    ...
    /// <summary>
    /// Defines the <see cref="GeoSpatialEncoderExperimental" />
    /// </summary>
    public class GeoSpatialEncoderExperimental : EncoderBase
    {
        /// <summary>
            ...
            if (IsRealCortexModel)
            {
                if (W < 1000 || W <= 0)
                {
                    throw new ArgumentException(
                        "Number of bits in the SDR (%d) must be greater than 2, and recommended >= 1000(use forced=True to override)");
                }
            }
        }
