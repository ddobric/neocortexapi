...
namespace NeoCortexApiSample
{
    ...
    public class SequenceLearning_DUY
    {
        public void Run()
        {
            ...
            List<double> inputValues1 = new List<double>(new double[] { 15.0, 5.0, 3.0, 4.0, 20.0, 3.0, 5.0 });

            RunExperiment(inputBits, cfg, encoder, inputValues, inputValues1);
        }

        /// <summary>
        ///
        /// </summary>
        private void RunExperiment(int inputBits, HtmConfig cfg, EncoderBase encoder, List<double> inputValues, List<double> inputValues1)
        {
        //double userInputs;
...
/*                Boolean done = false;
            while (mode != "1" && mode != "2")
            {
                Console.WriteLine($"Please choose a mode: 1 for no reset and 2 for with reset");
            ...
            while (done == false)
            {
                ...
                try
                {
                    userInputs = double.Parse(Console.ReadLine());
                    inputValues.Add(userInputs);
                } catch (Exception e)
                {
                    done = true;
                }
...
/*            int runTime = 2;
            ...
            for (int seq = 0; seq < runTime; seq++)
            {
                ...
                if (seq == 0)
                {
                    ...
                    numInputs = inputValues.Distinct<double>().ToList().Count;
                }
                else
                {
                    inputs = inputValues1.ToArray();
                    numInputs = inputValues1.Distinct<double>().ToList().Count;
                }*/
            ...
            for (int seq = 0; seq < runTime; seq++)
            {
                ...
                //TODO: The result it never reaches 100%. Why??????????
                    ...
                    if (accuracy == 100.0)
                    {
                        ...
                        if (cycle == maxCycles)
                        {
                            if (maxMatchCnt > globalMaxMatchCnt)
                            {
                                globalNearMaxMatchCnt = globalMaxMatchCnt;
                                globalMaxMatchCnt = maxMatchCnt;
                                lastCorrectCycle = cycle;
                            }
                            sw.Stop();
                            ...
                            Debug.WriteLine($"Maximum number of consecutive 100% correct: {globalMaxMatchCnt}");
                            Console.WriteLine($"Maximum number of consecutive 100% correct: {globalMaxMatchCnt}");
                            Debug.WriteLine($"Last correct cycle: {lastCorrectCycle}");
                            Console.WriteLine($"Last correct cycle: {lastCorrectCycle}");
                            Debug.WriteLine($"Second longest number of consecutive 100% correct: {globalNearMaxMatchCnt}");
                            Console.WriteLine($"Second longest number of consecutive 100% correct: {globalNearMaxMatchCnt}");
                            learn = false;
                            break;
                        }
                    }
