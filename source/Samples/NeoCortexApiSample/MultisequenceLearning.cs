...
namespace NeoCortexApiSample
{
    ...
    public class MultiSequenceLearning
    {
        ...
        public Predictor Run(Dictionary<string, List<double>> sequences)
        {
            ...
            var cfg = new HtmConfig(new int[] { inputBits }, new int[] { numColumns })
            {
            ...
            /*
             * Make it fetch the settings from the blob storage.
             */
        ...
        ///
        /// </summary>
        private Predictor RunExperiment(int inputBits, HtmConfig cfg, EncoderBase encoder,
            Dictionary<string, List<double>> sequences)
        {
            ...
            var sp = new SpatialPoolerMT(hpc);
            sp.Init(mem);
            tm.Init(mem);
            ...
            foreach (var sequenceKeyPair in sequences)
            {
                ...
                var previousInputs = new List<string> { "-1.0" };
                ...
                for (int i = 0; i < maxCycles; i++)
                {
                    ...
                    foreach (var input in sequenceKeyPair.Value)
                    {
                        ...
                        Console.WriteLine(
                            $"Cell SDR: {Helpers.StringifyVector(actCells.Select(c => c.Index).ToArray())}");
                        ...
                        if (lastPredictedValues.Contains(key))
                        {
                            matches++;
                            Console.WriteLine(
                                $"Match. Actual value: {key} - Predicted value: {lastPredictedValues.FirstOrDefault(key)}.");
                        }
                        else
                            Console.WriteLine(
                                $"Missmatch! Actual value: {key} - Predicted values: {String.Join(',', lastPredictedValues)}");
                        ...
                        if (lyrOut.PredictiveCells.Count > 0)
                        {
                            ...
                            {
                                Console.WriteLine(
                                    $"Current Input: {input} \t| Predicted Input: {item.PredictedInput} - {item.Similarity}");
                            }
                    ...
                    double maxPossibleAccuraccy = (double)((double)sequenceKeyPair.Value.Count - 1) /
                        (double)sequenceKeyPair.Value.Count * 100.0;

                    double accuracy = (double)matches / (double)sequenceKeyPair.Value.Count * 100.0;

                    Console.WriteLine(
                        $"Cycle: {cycle}\tMatches={matches} of {sequenceKeyPair.Value.Count}\t {accuracy}%");
                    ...
                    if (accuracy >= maxPossibleAccuraccy)
                    {
                        ...
                        if (maxMatchCnt >= 30)
                        {
                            sw.Stop();
                            Console.WriteLine(
                                $"Sequence learned. The algorithm is in the stable state after 30 repeats with with accuracy {accuracy} of maximum possible {maxMatchCnt}. Elapsed sequence {sequenceKeyPair.Key} learning time: {sw.Elapsed}.");
                    ...
                    {
                        Console.WriteLine(
                            $"At 100% accuracy after {maxMatchCnt} repeats we get a drop of accuracy with accuracy {accuracy}. This indicates instable state. Learning will be continued.");
                        maxMatchCnt = 0;
                    }
