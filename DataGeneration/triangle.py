...
def triangleCoord(centerPT, R):
    pt1 = (int(centerPT[0]-R*math.sqrt(3)/2),int(centerPT[1]+R/2))
    pt2 = (int(centerPT[0]),int(centerPT[1]-R))
    pt3 = (int(centerPT[0]+R*math.sqrt(3)/2),int(centerPT[1]+R/2))
...
video = VideoWriter('./'+experimentName+'/triangle.mp4', -1, float(FPS), (screenWidth, screenHeight))
...
for i in range(FPS*seconds):
    ...
    if(((triag_Point[0][1]+vectorTransform['y'])>screenHeight) or ((triag_Point[1][1]+vectorTransform['y'])<0)):
