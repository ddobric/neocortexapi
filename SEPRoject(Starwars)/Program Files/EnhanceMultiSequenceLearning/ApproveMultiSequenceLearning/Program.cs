...
namespace MultiSequenceLearning
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            List<SequenceString> sequencesString = ReadDataset(Path.Combine(basePath, DatasetFolder, DatasetFileName));
            //to read test dataset
            List<SequenceString> sequencesTestString = ReadDataset(Path.Combine(basePath, DatasetFolder, TestsetFileName));

            List<Sequence> sequences = new List<Sequence>();
            List<Sequence> sequencesTest = new List<Sequence>();
            foreach (var x in sequencesString)
            {
                sequences.Add(new Sequence() { name = x.name, data=HelperMethods.ConvertToAscii(x.data) });
            }
            foreach (var x in sequencesTestString)
            {
                sequencesTest.Add(new Sequence() { name = x.name, data = HelperMethods.ConvertToAscii(x.data) });
            }
        ...
        private static List<SequenceString> ReadDataset(string datasetPath)
        {
            try
            {
                Console.WriteLine($"Reading Dataset: {datasetPath}");
                return JsonConvert.DeserializeObject<List<SequenceString>>(File.ReadAllText(datasetPath));
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error reading dataset: {ex.Message}");
                return new List<SequenceString>();
            }
        }
